...
class ResidualNetwork(Network):
  ...
  def _build_custom(self):
    ...
    # Shared eye layers.
    conv1_e1 = layers.Conv2D(96, (3, 3), activation="relu", padding="SAME",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv1_e2 = layers.Conv2D(96, (3, 3), activation="relu", padding="SAME",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv1_e3 = layers.Conv2D(96, (3, 3), activation="relu", padding="SAME",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv1_e4 = layers.Conv2D(96, (3, 3), activation="relu", padding="SAME",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv1_e5 = layers.Conv2D(96, (1, 1), activation="relu", padding="SAME",
                             kernel_regularizer=self._l2, trainable=trainable)

    shortcut1 = layers.Conv2D(96, (1, 1), activation="relu",
                              kernel_regularizer=self._l2, trainable=trainable)
    norm1 = layers.BatchNormalization(trainable=trainable)

    pool1 = layers.MaxPooling2D(pool_size=11, strides=(8, 8))

    pad2 = layers.ZeroPadding2D(padding=(2, 2))
    conv2_e1 = layers.Conv2D(256, (3, 3), activation="relu",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv2_e2 = layers.Conv2D(256, (3, 3), activation="relu",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv2_e3 = layers.Conv2D(256, (1, 1), activation="relu",
                             kernel_regularizer=self._l2, trainable=trainable)

    shortcut2 = layers.Conv2D(256, (1, 1), activation="relu",
                              kernel_regularizer=self._l2, trainable=trainable)
    ...
    conv3_e1 = layers.Conv2D(384, (3, 3), activation="relu",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv3_e2 = layers.Conv2D(384, (1, 1), activation="relu",
                             kernel_regularizer=self._l2, trainable=trainable)
    conv3_e3 = layers.Conv2D(384, (1, 1), activation="relu",
                             kernel_regularizer=self._l2, trainable=trainable)

    shortcut3 = layers.Conv2D(384, (1, 1), activation="relu",
                              kernel_regularizer=self._l2, trainable=trainable)
    ...
    leye_conv4_e1 = conv4_e1(leye_norm3)
    ...
    reye_conv4_e1 = conv4_e1(reye_norm3)
